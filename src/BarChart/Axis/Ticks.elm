...
time n =
   custom <| \data axis ->
    [ set Tick.int Tick.format .timestamp (Values.time (Values.around n) axis) ]
...
floatCustom tick n =
  custom <| \data axis ->
    [ set tick toString toFloat (Values.float (Values.around n) axis) ]
...
timeCustom n f =
  custom <| \data axis ->
    [ set tick Tick.format .timestamp (Values.time axis) ]
